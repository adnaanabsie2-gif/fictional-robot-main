...
I chose to represent the PlayerTracker using a Map<String, Map<String, Integer>>.
The outer map stores player names, while the inner map stores each player’s
statistics (points, rebounds, assists, steals, and blocks).

This representation is effective because it allows for fast lookup and updates
of player statistics, and it closely matches the logical structure of the
component. Each player directly maps to their stats, making the kernel methods
...
- Designed kernel implementation for playertracker component
